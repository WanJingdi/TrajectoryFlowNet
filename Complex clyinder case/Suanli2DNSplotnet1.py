...
class TrajectoryNSNet():
    def __init__(self, B1):
        ...
        self.net_1.load_state_dict(torch.load("suanli2DNSnet1_2000.pth"))
...
xyuv_train = np.hstack([x_test, y_test, u_test, v_test])
...
plot_test_index = np.random.choice(200, 70, replace=False)
...
for i in range(int(len(plot_test_index))):
    ...
    plt.scatter(x0_test[index_plot[j]:index_plot[j]+delta_index[j]], y0_test[index_plot[j]:index_plot[j]+delta_index[j]], s=10, marker="o", c='red', label='Initial position',zorder=10)
...
ax.set_ylim(0, 0.2)
ax.set_aspect('equal')
fig.savefig('2DNS_all_particals'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
#####################################################################################
...
np.savetxt('2DNSl2trajectory.csv', relative_errors_xy, fmt='%.18f', delimiter=',', newline='\n')
...
np.savetxt('Case2_pearson_r_values.csv', pearson_r_xy,
