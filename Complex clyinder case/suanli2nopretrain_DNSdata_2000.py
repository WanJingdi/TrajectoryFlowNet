...
class TrajectoryNSNet():
    def __init__(self, B1, B2):
        ...
        self.net_2 = Net2(layers_2).to(device)
        # self.net_1.load_state_dict(torch.load("newsuanli2net1pretrain_12.24.pth"))
        # self.net_2.load_state_dict(torch.load("newsuanli2_elbowwithcylindernet2pretrain_small.pth"))
    ...
    def closure(self):
        ...
        total_loss =  100 * loss_x + loss_e + loss_u +  loss_f
        self.iter += 1
        if self.iter % 10 == 0:
            print(
    ...
    def train(self, traindata):
        ...
        print('End')
        torch.save(self.net_1.state_dict(), "suanli2DNSnet1_2000.pth")
        torch.save(self.net_2.state_dict(), "suanli2DNSnet2_2000.pth")
...
plot_test_index = np.random.choice(2000, 300, replace=False)

for i in range(int(len(plot_test_index))):

    j=plot_test_index[i]
    print(j)
    plt.scatter(x_net[index_plot[j]:index_plot[j]+delta_index[j]], y_net[index_plot[j]:index_plot[j]+delta_index[j]],s=0.1, marker="o", color='gray', alpha=1 ,label='trajectory', rasterized=True)
    plt.scatter(x0_train[index_plot[j]:index_plot[j]+delta_index[j]], y0_train[index_plot[j]:index_plot[j]+delta_index[j]], s=0.5, marker="o", c='red', label='Initial position',zorder=10, rasterized=True)

# plt.scatter(x_net, y_net, s=0.1, marker=",", color='gray', alpha=0.2 ,label='trajectory', rasterized=True)
# plt.scatter(x0_train, y0_train, s=0.1, marker=",", c='red', label='Initial position',zorder=5, rasterized=True)
...
ax.set_xlim(0, 1)
ax.set_ylim(0, 0.2)
fig.savefig('2_DNS_all_train_particles'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()


t_traindata_plot = t_net / (U / L)
t_traindata_plot = t_traindata_plot + t_start
...
U_train = ax.scatter(x_net[extrater_traindata_plot],y_net[extrater_traindata_plot], c=u_mag_train_plot, cmap='rainbow', s=1,  marker=',', alpha=0.9, edgecolors='none',vmin = 0, vmax = 0.3, rasterized=True)
...
ax.set_xlim(0, 1)
ax.set_ylim(0, 0.2)
fig.savefig('2_DNS_all_train_velo_mag_25'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
plt.scatter(xyt0_train[:,0:1],xyt0_train[:,1:2],s =2, marker="o")
...
train_data = addbatch(xyt0_train,xyuv_train, 20000)
