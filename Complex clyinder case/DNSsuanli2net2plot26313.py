...
class TrajectoryNSNet():
    def __init__(self,B):
        ...
        self.iter = 0
        self.net_2.load_state_dict(torch.load("suanli2DNSnet2_2000.pth"))
    ...
    def net_f(self, x, y, t1, B):
        ...
        f_u = u_t + u * u_x + v * u_y + p_x - (0.001003 / (998.2 * U * L)) * (u_xx + u_yy)
        f_v = v_t + u * v_x + v * v_y + p_y - (0.001003 / (998.2 * U * L)) * (v_xx + v_yy)
...
train_data = np.load('all_particle_data_with_pressure_filtered.npy')
...
if leak:
    print(f"LLeakage detected: {leak}")
    sys.exit(1)
...
#     if x_f_train[i]>0.04 or x_f_train[i]<0.02 :
#         extrater_f.append(i)
#
# x_f_train = np.delete(x_f_train, extrater_f, axis=0)
# y_f_train = np.delete(y_f_train, extrater_f, axis=0)
# x_f_train = [i-0.02 for i in x_f_train]
...
delta_t_train = [random.uniform(0, 0.02 * (U / L)) for _ in range(N_f)]
...
p_par = p_net * (998.2 * (U ** 2))
p_test_pred = p_test_pred * (998.2 * (U ** 2))
...
wangge = np.load('DNS_case2_25.npy')
...
wangge_u = np.array(wangge[:,2:3]).flatten()[:, None]
wangge_x = np.array(wangge[:,0:1]).flatten()[:, None]
wangge_y = np.array(wangge[:,1:2]).flatten()[:, None]
wangge_p = np.array(wangge[:,4:5]).flatten()[:, None]
wangge_v = np.array(wangge[:,3:4]).flatten()[:, None]
wangge_p = [i * 998.2  for i in wangge_p]
wangge_p = np.array(wangge_p)
...
# V_pred = ax1.scatter(wangge_x,wangge_y, c=wangge_u, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none')
# ax1.set_xlabel('$x$', size=10)
# ax1.set_ylabel('$y$', size=10)
#
#
# fig.subplots_adjust(hspace=10.0)
# fig.colorbar(V_pred, ax=ax1, fraction=0.03, shrink=0.5)
# plt.vlines(0.08,0.1,0, colors='r', label='-',lw=2)
# plt.vlines(0.18,0.1,0, colors='r', label='-',lw=2)
# plt.hlines(0.1,0.08,0.18, colors='r', label='-',lw=2)
# plt.hlines(0,0.08,0.18, colors='r', label='-',lw=2)
# plt.show()


# extrater_plot = []
# for i in range(len(wangge_x)):
#     if wangge_x[i]<0.08 or wangge_x[i]>0.18:
#         extrater_plot.append(i)
#
#
#
# wangge_x = np.delete(wangge_x, extrater_plot, axis=0)
# wangge_y = np.delete(wangge_y, extrater_plot, axis=0)
# wangge_p = np.delete(wangge_p, extrater_plot, axis=0)
# wangge_u = np.delete(wangge_u, extrater_plot, axis=0)
# wangge_v = np.delete(wangge_v, extrater_plot, axis=0)
#
# wangge_x = [i-0.06 for i in wangge_x]
# wangge_y = [i-0.10 for i in wangge_y]
...
xiuzheng = []
for i in range(len(wangge_x)):
    if wangge_x[i] > 0.99 and wangge_y[i]>0.17 and wangge_y[i]<0.19:
        xiuzheng.append(i)

id_xiuzheng = xiuzheng[0]


xiuzhengP_x = wangge_x[id_xiuzheng]
xiuzhengP_y = wangge_y[id_xiuzheng]
xiuzhengP_p = 0.0
xiuzhengP_t = float(0.5 * (U / L))
xiuzhengliang = p_test_pred[id_xiuzheng] - xiuzhengP_p


# p_test_pred = p_test_pred - xiuzhengliang
p_test_pred = [i-xiuzhengliang for i in p_test_pred]
p_test_pred = np.array(p_test_pred)
wangge_u = wangge_u * U
u_test_pred = u_test_pred * U
wangge_v = wangge_v * U
v_test_pred = v_test_pred * U
wangge_p = wangge_p * (998.2 * (U ** 2))
p_test_pred = p_test_pred * (998.2 * (U ** 2))
print(p_test_pred[id_xiuzheng])
print(wangge_p[id_xiuzheng])
print(1/(0.001003 / (998.2 * U * L)))
...
U_pred = ax2.scatter(wangge_x,wangge_y, c=u_pred_mag, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=0,vmax=0.3, rasterized=True)
U_true = ax1.scatter(wangge_x,wangge_y, c=wangge_u_mag, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=0,vmax=0.3, rasterized=True)
ax1.set_xticks([])
ax1.set_yticks([])
...
fig.savefig('2DNS_u_mag_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()


fig, ax = plt.subplots(figsize=(12,6))
error_u = ax.scatter(wangge_x,wangge_y, c=u_pred_mag-wangge_u_mag, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', vmin=-0.3, vmax=0.3, rasterized=True)
...
ax.set_aspect('equal')
fig.savefig('2DNS_error_u_mag_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
ax1= fig.add_subplot(121,aspect='equal')
ax2= fig.add_subplot(122,aspect='equal')
U_pred = ax2.scatter(wangge_x,wangge_y, c=u_test_pred, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=-0.27,vmax=0.27, rasterized=True)
U_true = ax1.scatter(wangge_x,wangge_y, c=wangge_u, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=-0.27,vmax=0.27, rasterized=True)
# ax1.set_xlabel('$x$', size=10)
...
fig.savefig('2DNS_u_25_25s' + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
ax1= fig.add_subplot(121,aspect='equal')
ax2= fig.add_subplot(122,aspect='equal')
V_pred = ax2.scatter(wangge_x,wangge_y, c=v_test_pred, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=-0.26,vmax=0.26, rasterized=True)
V_true = ax1.scatter(wangge_x,wangge_y, c=wangge_v, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin=-0.26,vmax=0.26, rasterized=True)
...
cbar.ax.tick_params(labelsize=8)  # 设置刻度标签字体大小
fig.savefig('2DNS_v_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
ax1= fig.add_subplot(121,aspect='equal')
ax2= fig.add_subplot(122,aspect='equal')
P_pred = ax2.scatter(wangge_x,wangge_y, c=p_test_pred, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin = -50, vmax=50, rasterized=True)
P_true = ax1.scatter(wangge_x,wangge_y, c=wangge_p, cmap='rainbow', s=1, marker=',', alpha=0.9, edgecolors='none',vmin = -50, vmax=50, rasterized=True)
ax1.set_title('p true t=25s', fontsize = 10)
ax2.set_title('p predict t=25s', fontsize = 10)
ax1.set_xticks([])
ax1.set_yticks([])
for spine in ax1.spines.values():
    spine.set_visible(False)
...
for spine in ax2.spines.values():
    spine.set_visible(False)
fig.subplots_adjust(hspace=10.0)
clim = P_true.get_clim()
# 创建包含五个刻度位置的数组
ticks_positions = np.linspace(clim[0], clim[1], 5)

# 添加颜色条，并指定其宽度、长度、刻度位置及格式
cbar = fig.colorbar(P_true, ax=[ax1, ax2], fraction=0.046, pad=0.04, ticks=ticks_positions, format='%.2f')
cbar.ax.tick_params(labelsize=8)  # 设置刻度标签字体大小
fig.savefig('2DNS_p_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()


######################################################
fig, ax = plt.subplots(figsize=(12,6))
error_u = ax.scatter(wangge_x,wangge_y, c=u_test_pred-wangge_u, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', vmin=-wangge_u.max(), vmax=wangge_u.max(), rasterized=True)
# ax.scatter(filtered_data_x, filtered_data_y, s=10, marker="x", c='gray', label='Particle position',zorder=10, alpha=0.6, rasterized=True)
# plt.xticks(size = 20)
# plt.yticks(size = 20)
divider = make_axes_locatable(ax)
...
clim = error_u.get_clim()
# 创建包含五个刻度位置的数组
ticks_positions = np.linspace(clim[0], clim[1], 5)

# 添加颜色条，并指定其宽度、长度、刻度位置及格式
cbar = fig.colorbar(error_u, fraction=0.046, pad=0.04, ticks=ticks_positions, format='%.2f')
# cbar.ax.tick_params(labelsize=15)  # 设置刻度标签字体大小
...
for spine in ax.spines.values():
    spine.set_visible(False)
ax.legend()
# ax.set_title('error_u t=25s', fontsize = 30)
ax.set_aspect('equal')
fig.savefig('2DNS_error_u_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()

fig, ax = plt.subplots(figsize=(12,6))
error_v = ax.scatter(wangge_x,wangge_y, c=v_test_pred-wangge_v, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', vmin=wangge_v.min(), vmax=-wangge_v.min(), rasterized=True)
# ax.scatter(filtered_data_x, filtered_data_y, s=10, marker="x", c='gray', label='Particle position',zorder=10, alpha=0.6, rasterized=True)
# plt.xticks(size = 20)
# plt.yticks(size = 20)
divider = make_axes_locatable(ax)
clim = error_v.get_clim()
# 创建包含五个刻度位置的数组
ticks_positions = np.linspace(clim[0], clim[1], 5)

# 添加颜色条，并指定其宽度、长度、刻度位置及格式
cbar = fig.colorbar(error_v, fraction=0.046, pad=0.04, ticks=ticks_positions, format='%.2f')
# cbar.ax.tick_params(labelsize=15)  # 设置刻度标签字体大小
# ax.set_xlabel('$x$', size=30)
# ax.set_ylabel('$y$', size=30)
ax.set_xticks([])
...
for spine in ax.spines.values():
    spine.set_visible(False)
ax.legend()
# ax.set_title('error_v t=25s', fontsize = 30)
ax.set_aspect('equal')
fig.savefig('2DNS_error_v_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()

fig, ax = plt.subplots(figsize=(12,6))
error_p = ax.scatter(wangge_x,wangge_y, c=p_test_pred-wangge_p, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', vmin=-50, vmax=50, rasterized=True)
# ax.scatter(filtered_data_x, filtered_data_y, s=10, marker="x", c='gray', label='Particle position',zorder=10, alpha=0.6, rasterized=True)
# plt.xticks(size = 20)
# plt.yticks(size = 20)
divider = make_axes_locatable(ax)
...
for spine in ax.spines.values():
    spine.set_visible(False)
ax.legend()
# ax.set_title('error_p t=25s', fontsize = 30)
ax.set_aspect('equal')
fig.savefig('2DNS_error_p_25_25s'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()

# fig, ax = plt.subplots(figsize=(12,6))
# error_p = ax.scatter(wangge_x,wangge_y, c=p_test_pred, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', rasterized=True)
# plt.xticks(size = 20)
# plt.yticks(size = 20)
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.30)
...
# ax.set_xlabel('$x$', size=30)
# ax.set_ylabel('$y$', size=30)
# ax.set_title('p_pred t=25s', fontsize = 30)
# ax.set_aspect('equal')
# plt.show()
#
# fig, ax = plt.subplots(figsize=(12,6))
# error_p = ax.scatter(wangge_x,wangge_y, c=wangge_p, cmap='seismic', s=2, marker=',', alpha=0.9, edgecolors='none', rasterized=True)
# plt.xticks(size = 20)
# plt.yticks(size = 20)
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.30)
...
# ax.set_xlabel('$x$', size=30)
# ax.set_ylabel('$y$', size=30)
# ax.set_title('p_true t=25s', fontsize = 30)
# ax.set_aspect('equal')
# plt.show()
...
# UU_true = np.squeeze(U_true, axis=2)
# error_u = U-UU_true
# h = ax.imshow(error_u, interpolation='nearest', cmap='seismic',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal', vmin=-wangge_u.max(), vmax=wangge_u.max())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
...
# ax.set_title('error_u', fontsize=20)
# plt.show()
#
# fig = plt.figure(figsize=(9, 5))
# ax = fig.add_subplot(111)
# VV_true = np.squeeze(V_true, axis=2)
# error_v = V-VV_true
# h = ax.imshow(error_v, interpolation='nearest', cmap='seismic',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal',vmin=wangge_v.min(), vmax=-wangge_v.min())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
# cbar = fig.colorbar(h, cax=cax)
# cbar.ax.tick_params(labelsize=15)
# ax.set_xlabel('$x$', fontsize=15)
# ax.set_ylabel('$y$', fontsize=15)
# ax.set_title('error_v', fontsize=20)
# plt.show()
#
# fig = plt.figure(figsize=(9, 5))
# ax = fig.add_subplot(111)
# PP_true = np.squeeze(P_true, axis=2)
# error_p = P-PP_true
# h = ax.imshow(error_p, interpolation='nearest', cmap='seismic',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal',vmin=wangge_p.min(), vmax=-wangge_p.min())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
# cbar = fig.colorbar(h, cax=cax)
# cbar.ax.tick_params(labelsize=15)
# ax.set_xlabel('$x$', fontsize=15)
# ax.set_ylabel('$y$', fontsize=15)
# ax.set_title('error_p', fontsize=20)
# plt.show()
# #######################################################
#
# ###########################
...
# UU_true = np.squeeze(U_true, axis=2)
# error_u = U-UU_true
# h = ax.imshow(error_u, interpolation='nearest', cmap='rainbow',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal', vmin=wangge_u.min(), vmax=wangge_u.max())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
# cbar = fig.colorbar(h, cax=cax)
# cbar.ax.tick_params(labelsize=15)
...
# VV_true = np.squeeze(V_true, axis=2)
# error_v = V-VV_true
# h = ax.imshow(error_v, interpolation='nearest', cmap='rainbow',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal',vmin=wangge_v.min(), vmax=wangge_v.max())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
# cbar = fig.colorbar(h, cax=cax)
# cbar.ax.tick_params(labelsize=15)
...
# PP_true = np.squeeze(P_true, axis=2)
# error_p = P-PP_true
# h = ax.imshow(error_p, interpolation='nearest', cmap='rainbow',
#               extent=[wangge_x.min(), wangge_x.max(), wangge_y.min(), wangge_y.max()],
#               origin='lower', aspect='equal',vmin=wangge_p.min(), vmax=wangge_p.max())
# divider = make_axes_locatable(ax)
# cax = divider.append_axes("right", size="5%", pad=0.10)
# cbar = fig.colorbar(h, cax=cax)
# cbar.ax.tick_params(labelsize=15)
...
df_true.to_csv('2DNStrue_velocity_kde.csv', index=False)
df_pred.to_csv('2DNSpredicted_velocity_kde.csv', index=False)
df_error.to_csv('2DNSerror_kde.csv', index=False)
