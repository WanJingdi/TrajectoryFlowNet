...
class TrajectoryNSNet():
    def __init__(self, B1, B2):
        ...
        # self.net_1.load_state_dict(torch.load("suanli4net1pretrain_test.pth"))
        # self.net_2.load_state_dict(torch.load("suanli4net2pretrain_test.pth"))
    ...
    def net_f(self, x0, y0, t1, t0, t2, B1, B2):
        ...
        self.RE1 = 0.0005  * torch.sigmoid(lambda_1)
    ...
    def closure(self):
        ...
        total_loss =   1 * loss_x +   0.5 * loss_e +  loss_u +   0.0001 * loss_f
        ...
        # if self.iter % 100 == 0:
            # torch.save(self.net_1.state_dict(), f"suanli4net1nopretrain_all_aug_{self.iter}.pth")
            # torch.save(self.net_2.state_dict(), f"suanli4net2nopretrain_all_aug_{self.iter}.pth")
    ...
    def train(self, traindata):
        ...
        # torch.save(self.net_1.state_dict(), "suanli4net1nopretrain_all_aug_final.pth")
        # torch.save(self.net_2.state_dict(), "suanli4net2nopretrain_all_aug_final.pth")
...
delta_index_sort = np.sort(delta_index)
...
plt.scatter(x_net,y_net, c = t_net / (U / L),s =2, marker="o")
...
idx_auto = np.random.choice(x_train_auto.shape[0], int(3 * len(x_net)), replace=False)
x0_net_auto = x_train_auto[idx_auto]
y0_net_auto = y_train_auto[idx_auto]
t0_net_auto = t_train_auto[idx_auto]
x_net_auto = x_train_auto[idx_auto]
...
###########轨迹切片数据增强############################
def trajectory_augmentation(x0_train, y0_train, t0_train,
                            x_net, y_net, t_net,
                            u_net, v_net,
                            min_time_steps_list=[2]):
    """
    轨迹数据增强：提取不同长度的轨迹切片作为新的训练样本

    关键特性：
    - 支持多种切片长度（时间步数），例如[2, 3, 4]
    - 只有当轨迹长度足够时，才会生成对应长度的切片
    - 对于每个轨迹切片，所有时间步共享相同的x0/y0/t0（切片起始点）
    - x0_train, y0_train, t0_train 的行数与 x_net, y_net, t_net 完全一致

    参数:
    min_time_steps_list: 要生成的切片长度列表，例如[2, 3, 4]
                        2表示切片包含2个时间步[t0, t1]
                        3表示切片包含3个时间步[t0, t1, t2]
                        4表示切片包含4个时间步[t0, t1, t2, t3]
    ...
    # 转换为numpy数组并展平
    x0_train = np.array(x0_train).flatten()
    y0_train = np.array(y0_train).flatten()
    t0_train = np.array(t0_train).flatten()
    x_net = np.array(x_net).flatten()
    y_net = np.array(y_net).flatten()
    t_net = np.array(t_net).flatten()
    u_net = np.array(u_net).flatten()
    v_net = np.array(v_net).flatten()

    # 识别轨迹边界：相同t0_train的连续数据属于同一条轨迹
    trajectory_starts = [0]
    for i in range(1, len(t0_train)):
        if t0_train[i] != t0_train[i - 1]:
            trajectory_starts.append(i)
    trajectory_starts.append(len(t0_train))
    ...
    for i in range(len(trajectory_starts) - 1):
        ...
        end_idx = trajectory_starts[i + 1]

        # 获取当前轨迹数据
        traj_t = t_net[start_idx:end_idx]
        traj_x = x_net[start_idx:end_idx]
        traj_y = y_net[start_idx:end_idx]
        traj_u = u_net[start_idx:end_idx]
        traj_v = v_net[start_idx:end_idx]

        # 获取原始起始点信息
        orig_x0 = x0_train[start_idx]
        orig_y0 = y0_train[start_idx]
        orig_t0 = t0_train[start_idx]

        # 检查轨迹长度
        trajectory_length = len(traj_t)

        # 对每种切片长度进行处理
        for slice_length in min_time_steps_list:
            # 如果轨迹不够长，跳过这个切片长度
            if trajectory_length < slice_length:
                continue

            # 生成该切片长度的所有可能切片
            # 从轨迹的每个位置开始，只要剩余长度足够
            for start_pos in range(trajectory_length - slice_length + 1):
                # 获取切片的结束位置
                end_pos = start_pos + slice_length

                # 提取切片数据
                slice_x = traj_x[start_pos:end_pos]
                slice_y = traj_y[start_pos:end_pos]
                slice_t = traj_t[start_pos:end_pos]
                slice_u = traj_u[start_pos:end_pos]
                slice_v = traj_v[start_pos:end_pos]
                ...
                # 对切片中的每个时间步，添加对应的x0/y0/t0（都相同）
                for k in range(len(slice_x)):
                    new_x0_train.append(slice_x0)
                    new_y0_train.append(slice_y0)
                    new_t0_train.append(slice_t0)
                    new_x_net.append(slice_x[k])
                    new_y_net.append(slice_y[k])
                    new_t_net.append(slice_t[k])
                    new_u_net.append(slice_u[k])
                    new_v_net.append(slice_v[k])

    # 转换为numpy数组并重塑为列向量
    new_x0_train = np.array(new_x0_train).flatten()[:, None]
    new_y0_train = np.array(new_y0_train).flatten()[:, None]
    new_t0_train = np.array(new_t0_train).flatten()[:, None]
    new_x_net = np.array(new_x_net).flatten()[:, None]
    new_y_net = np.array(new_y_net).flatten()[:, None]
    new_t_net = np.array(new_t_net).flatten()[:, None]
    new_u_net = np.array(new_u_net).flatten()[:, None]
    new_v_net = np.array(new_v_net).flatten()[:, None]

    return (new_x0_train, new_y0_train, new_t0_train,
            new_x_net, new_y_net, new_t_net,
            new_u_net, new_v_net)


# 使用示例
print("原始数据形状:")
print(f"x0_train: {x0_train.shape}, x_net: {x_net.shape}")

# 进行轨迹增强，支持切片长度为2,3,4
(x0_train_aug, y0_train_aug, t0_train_aug,
 x_net_aug, y_net_aug, t_net_aug,
 u_net_aug, v_net_aug) = trajectory_augmentation(
    x0_train, y0_train, t0_train,
    x_net, y_net, t_net,
    u_net, v_net,
    min_time_steps_list=[2,5,10,15,20,25,30,35,40,45,50]

)


idx_aug = np.random.choice(x0_train_aug.shape[0], 18000, replace=False)
x0_train_aug = x0_train_aug[idx_aug]
y0_train_aug = y0_train_aug[idx_aug]
t0_train_aug = t0_train_aug[idx_aug]
x_net_aug = x_net_aug[idx_aug]
y_net_aug = y_net_aug[idx_aug]
t_net_aug = t_net_aug[idx_aug]
u_net_aug = u_net_aug[idx_aug]
v_net_aug = v_net_aug[idx_aug]

print("\n增强后数据形状:")
print(f"x0_train: {x0_train_aug.shape}")
print(f"x_net: {x_net_aug.shape}")
print(f"验证: x0_train行数 == x_net行数: {x0_train_aug.shape[0] == x_net_aug.shape[0]}")


x0_train =  np.vstack((x0_train,x0_train_aug))
y0_train =  np.vstack((y0_train,y0_train_aug))
t0_train =  np.vstack((t0_train,t0_train_aug))
x_net =  np.vstack((x_net,x_net_aug))
y_net =  np.vstack((y_net,y_net_aug))
t_net =  np.vstack((t_net,t_net_aug))
u_net =  np.vstack((u_net,u_net_aug))
v_net =  np.vstack((v_net,v_net_aug))
...
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.05'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
extrater_traindata_plot = []
for i in range(len(t_net)):
    if t_traindata_plot[i]<0.601 and t_traindata_plot[i]>0.599:
        extrater_traindata_plot.append(i)
u_mag_train_plot = (u_traindata_plot[extrater_traindata_plot] ** 2 + v_traindata_plot[extrater_traindata_plot] ** 2) ** 0.5
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
...
plt.xlim([0, 1])  # 设置x轴范围为0到10
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.6'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
plt.xlim([0, 1])  # 设置x轴范围为0到10
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.10'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
extrater_traindata_plot = []
for i in range(len(t_net)):
    if t_traindata_plot[i]<0.301 and t_traindata_plot[i]>0.299:
        extrater_traindata_plot.append(i)
u_mag_train_plot = (u_traindata_plot[extrater_traindata_plot] ** 2 + v_traindata_plot[extrater_traindata_plot] ** 2) ** 0.5
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
...
plt.xlim([0, 1])  # 设置x轴范围为0到10
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.30'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
extrater_traindata_plot = []
for i in range(len(t_net)):
    if t_traindata_plot[i]<0.501 and t_traindata_plot[i]>0.499:
        extrater_traindata_plot.append(i)
u_mag_train_plot = (u_traindata_plot[extrater_traindata_plot] ** 2 + v_traindata_plot[extrater_traindata_plot] ** 2) ** 0.5
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
...
plt.xlim([0, 1])  # 设置x轴范围为0到10
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.5'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
...
extrater_traindata_plot = []
for i in range(len(t_net)):
    if t_traindata_plot[i]<0.951 and t_traindata_plot[i]>0.949:
        extrater_traindata_plot.append(i)
u_mag_train_plot = (u_traindata_plot[extrater_traindata_plot] ** 2 + v_traindata_plot[extrater_traindata_plot] ** 2) ** 0.5
fig = plt.figure(figsize=(6, 6), dpi=300)
ax = fig.add_subplot(111)
...
plt.xlim([0, 1])  # 设置x轴范围为0到10
plt.ylim([0, 1])
fig.savefig('4_all_train_velo_mag_0.95'  + ".svg", bbox_inches='tight', dpi=600, pad_inches=0.1)
plt.show()
####################################################################
...
# xyt0_train = xyt0_train[:32000,:]
# xyuv_train = xyuv_train[:32000,:]
#jiangcaiyang
# M = []
...
plt.scatter(x0_train,y0_train, c = t0_train / (U / L),s =2, marker="o")
ax.set_xlabel('$x$', size=20)
ax.set_ylabel('$y$', size=20)
ax.tick_params(labelsize=15)
plt.colorbar()
...
train_data = addbatch(xyt0_train,xyuv_train, 3500000000)
